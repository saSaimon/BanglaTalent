...
class EditProfileForCandidate(Page):
    ...
    EditSummaryIcon = (By.XPATH,
                       "(//*[name()='svg'][@class='absolute cursor-pointer top-4 right-4 text-secondary-foreground hover:text-gray-500'])[2]")
    SummaryField = (By.XPATH, "(//textarea[@id='summary'])")
    CancelButton = (By.XPATH, "(//button[normalize-space()='Cancel'])[1]")
    SaveChangeButton = (By.XPATH, "(//button[normalize-space()='Save Changes'])[1]")
    ...
    def edit_profile_picture(self):
        self.find_element(*self.UploadProfile).send_keys('F:/logo.png')
        time.sleep(10)
    ...
    def edit_summary_icon(self):
        self.click(*self.EditSummaryIcon)

    def enter_summary(self, text):
        self.find_element(*self.SummaryField).clear()
        self.input_text(text, *self.SummaryField)
